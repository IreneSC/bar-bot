...
while toc(overall_timer) < duration
    if (mod(index, camera_rate) == 0)
        [centroidx, centroidy, isFaceFirstDetected, isFaceDetected, numPts, oldPoints, bboxPoints] = faceTrack(faceDetector, histogramTracker, cam, videoPlayer, numPts,oldPoints, bboxPoints);
    end
